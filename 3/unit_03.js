// Ссылка на урок: https://itgid.info/unit/javascript-2?unit=if-else

// Task 1
// При нажатии кнопки .b-1 срабатывает функция f1. Функция должна прочитать содержимое .i-1 и сравнить его с числом 4 (сравнение ==). Результат сравнения - true или false выведите в .out-1.

function f1(){
   document.querySelector('.out-1').textContent = document.querySelector('.i-1').value == 4;
}
...
function f3(){
   ...
   document.querySelector(".out-3").textContent = Math.max(a, b)
}
...
function f4(){
   let age = new Date().getFullYear() - document.querySelector('.i-4').value;
   document.querySelector('.out-4').textContent = (age >= 18) ? 1 : 0;
}
...
function f5(){
   let num = Number(document.querySelector('.i-5').value);

   document.querySelector('.out-5').textContent = (num === 0) ? 0 : (num < 0) ? 'm' : 1;
}
...
function f6(){
   ...
   document.querySelector('.out-6').textContent = (num % 2 === 0) ? 'even ' : 'odd';
}
...
function f7(){
   let num1 = Number(document.querySelector('.i-71').value);
   let num2 = Number(document.querySelector('.i-72').value);
...
function f8(){
   let selected = Number(document.querySelector('.s-8').value);
   let label;
   ...
   document.querySelector('.out-8').textContent = label;
}
...
function f9(){
   let apart_num = Number(document.querySelector('.i-9  ').value);
   let entr_num;

   if (between(1, 32, apart_num)) {
      entr_num = 1;
   } else if (between(33, 43, apart_num)) {
      entr_num = 2;
   } else if (between(44, 64, apart_num)) {
      entr_num = 3;
   } else {
      entr_num = 0;
   }

   document.querySelector('.out-9').textContent = entr_num;
}
...
function f10(){
   let v = document.querySelector('.s-100').value;

   document.querySelector('.out-10').textContent = v;
}
...
function f11(){
   let v = document.querySelector('.s-110').value;

   document.querySelector('.out-11').textContent = v;
}
...
function f14(){
   ...
   let sign = document.querySelector('.s-143').value;

   document.querySelector('.out-14').textContent = eval(a + sign + b)
}
...
function f15(){
   ...
   document.querySelector('.out-15').textContent = eval(a + sign + b)
}
